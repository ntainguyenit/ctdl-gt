...
const DoublyLinkedList = () => {
  ...
  const addToHead = () => {
    ...
    setInsertValue("");
    toast({ title: "Thành công", description: "Đã thêm phần tử vào đầu danh sách" });
  };
  ...
  const addToTail = () => {
    ...
    setInsertValue("");
    toast({ title: "Thành công", description: "Đã thêm phần tử vào cuối danh sách" });
  };
  ...
  return (
    <div className="space-y-8 p-6 bg-white rounded-lg shadow-md">
      <h2 className="text-2xl font-bold text-center text-blue-600">Danh Sách Liên Kết Đôi</h2>
      ...
      <div className="grid gap-4">
        <div className="flex gap-2">
          ...
          />
          <Button onClick={addToHead}>Thêm vào đầu</Button>
          <Button onClick={addToTail}>Thêm vào cuối</Button>
        </div>
        ...
        <AnimatePresence>
          <div className="flex items-center justify-start min-h-[100px] gap-2">
            {list.map((node, index) => (
              ...
              >
                {index > 0 && <ArrowLeft className="text-blue-600 mx-1" />}
                <div className="w-12 h-12 flex items-center justify-center border-2 border-blue-600 rounded-lg bg-white">
                  {node.value}
                </div>
                {index < list.length - 1 && <ArrowRight className="text-blue-600 mx-1" />}
              </motion.div>
            ))}
          </div>
        </AnimatePresence>
